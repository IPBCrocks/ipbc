...
Core tests take longer than any other IPBC tests, due to the high amount of computational work involved in validating core components.
...
To run only IPBC's core tests (after building):
...
To run only IPBC's crypto tests (after building):
...
To run only IPBC's hash tests (after building):
...
To run only IPBC's performance tests (after building):
...
To run only IPBC's unit tests (after building):
